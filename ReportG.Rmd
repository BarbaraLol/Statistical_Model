...
```
The full model is significantly better than the null model, but some of the variables are not statistically significant, except for glucose, for which the level 1 is not significant while the level 2 is, but it's negatively correlated with the target variable. This looks like a selection bias problem: it's likely that people with very high glucose levels have diabetes, so it's possible that 
