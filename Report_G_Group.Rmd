...
```{r}
# Create a copy without the excluded variables
df_gam<-df %>% select(-gender, -gluc, -height)

# Perform a train-test split (80/20)
set.seed(42) # For reproducibility
trainIndex <- createDataPartition(df_gam$cardio, p = 0.8, list = FALSE)
df_train_gam <- df_gam[trainIndex, ]
df_test_gam <- df_gam[-trainIndex, ]

# Define the formula for the GAM model
gam_formula <- as.formula("cardio ~ s(age) + s(weight) + s(ap_hi) + s(ap_lo) + s(BMI) + cholesterol + smoke + alco + active")

# Fit the GAM model
gam_model <- gam(gam_formula, data = df_gam, family = binomial(link = logit))

# Summary of the GAM model
summary(gam_model)

# Plot the partial effect of numerical variables
par(mfrow=c(2, 3))
plot(gam_model, pages = 1, col = "lightblue", main = "Partial Effect of Numerical Variables")
plot(gam_model, scale = 0, col = "lightblue", main = "Partial Effect of Numerical Variables") #stesso grafico, diversa ordinata

# Create partial dependence plots
pdp_cholesterol <- partial(gam_model, pred.var = "cholesterol", grid.resolution = 100)
pdp_smoke <- partial(gam_model, pred.var = "smoke", grid.resolution = 100)
pdp_alco <- partial(gam_model, pred.var = "alco", grid.resolution = 100)
pdp_active <- partial(gam_model, pred.var = "active", grid.resolution = 100)

# Plot the partial dependence plot for 'gender'
plot(pdp_cholesterol)
plot(pdp_smoke)
plot(pdp_alco)
plot(pdp_active)
```
